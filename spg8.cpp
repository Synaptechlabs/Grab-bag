...
public:
  ...
  void append(const std::shared_ptr<Node> &node) {
    if (!head) {
      ...
      // Reuse existing link if it exists, otherwise create new
      if (!tail->next.count(id)) {
        tail->next[id] = node;
        node->prev[id] = tail;
        tail->forwardFreq[id]++;
...
public:
  ...
  void learnInput(const std::string &input, const std::string &pathID) {
    ...
    // Seed activation: strongest to weakest
    float activation = 1.0f;
    for (auto it = inputNodes.begin(); it != inputNodes.end(); ++it) {
      (*it)->receiveInput(activation);
      activation *= 0.9f; // decay for each token
    }
    lastInputNode = inputNodes.empty() ? nullptr : inputNodes.back();
  }
  ...
  std::string generateResponse() {
    ...
    while (maxNode && !visited[maxNode->value]) {
      ...
      for (const auto &[_, target] : maxNode->next) { // ignore path ID
        float score = target->activation;
        if (score > maxScore) {
          maxScore = score;
          nextNode = target;
        }
