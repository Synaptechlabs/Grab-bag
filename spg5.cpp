...
class Engine {
public:
  void addNode(const std::shared_ptr<Node> &node) { allNodes.push_back(node); }
...
int main() {
  ...
  Engine engine;
  for (const auto &n : {the, a, she, cat, sat, has, four, legs, on, mat, chair,
                        talked, to, her}) {
    engine.addNode(n);
  }

  cat->receiveInput(2.0f);

  std::cout << "\nActivation Ticks:\n";
  for (int i = 0; i < 5; ++i) {
    engine.tick();
    engine.printActivations(i);
  }
